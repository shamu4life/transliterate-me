...
### vendor.sh, for refreshing the bundled binaries
...
`./vendor.sh` (needs Node/npm) only re-fetches or updates these files; you do
...
(`300` becomes "three hundred") rather than passed through.
...
1. **Text → pronunciation.** Two engines, chosen per source language in
   `src/lang/index.js`:
   - **English** (`engine: 'dict'`) is looked up in the bundled CMU Pronouncing
   ...
   - **Other 13 languages** (`engine: 'espeak'`) are phonemized to IPA by the
...
2. **Normalize to ARPABET.** Everything downstream operates on ARPABET
   ...
   phoneme, but vowel length survives as a `ː` token, so Katakana can
   add a chōonpu.
...
- **Plain ES modules, no framework, no transpiler.** Source files are served to
  ...
  Keep them runnable both in the browser and under `node --test`, with no
  bundler-only syntax and no `node_modules` imports in `public/`.
