...
### Added
- Forward: numerals are now spoken instead of dropped. Digits in the input are
...
- Forward: broad numeric coverage. Integers, decimals and negatives; both
  US (`1,000.50`) and European (`1.000,50`) grouping; years, read
  idiomatically as *nineteen ninety-nine*; ordinals (`1st`, `2nd`, and so on);
  currency (`$ £ € ¥`, with yen invariant, so *500 yen* rather than *500 yens*);
  percent; and fractions (`1/2` becomes *one half*). The European decimal
...
- Forward: English is pronounced from the bundled CMU Pronouncing
  Dictionary, with a rule-based fallback for unknown words, which is instant and
...
- Romanize: paste any of those 7 world scripts and romanize it back into the
  Latin alphabet. Chinese becomes Pīnyīn with tone marks (context-aware polyphones via
  pinyin-pro), Japanese becomes Hepburn romaji (full kanji + kana via kuroshiro +
  kuromoji), Korean becomes Revised Romanization, plus practical transliteration for
...
- UI: a two-tab client-side app (Forward and Romanize) that picks the source
  ...
  you type.
- Privacy: runs 100% client-side, with no backend, no build step, and no external
...
- Quality: every language and script pair carries an honest quality signal
  ...
  rendering versus a best-effort approximation. Chinese forward output is a
  nearest-Mandarin-syllable guess, for instance, and unwritten short vowels in
  unvocalized Arabic or Hebrew can't be recovered.
