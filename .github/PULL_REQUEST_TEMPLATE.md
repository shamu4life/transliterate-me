...
- [ ] New feature or capability: new script, source language, or romanizer (→ `MINOR`)
...
- [ ] Invariants still green: every ARPABET consonant produces output in every script, and Chinese output has no Latin pinyin leakage. Or N/A
...
- [ ] Files still run in both the browser and `node --test`, with no `node_modules` imports in `public/` and no bundler-only syntax. Or N/A
