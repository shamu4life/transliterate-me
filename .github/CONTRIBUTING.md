...
The two large binaries (espeak-ng's WASM plus the kuromoji dictionary, ~36 MB) ship committed in the repo (`public/vendor/`), so everything works out of the box. `./vendor.sh` (needs Node/npm) only re-fetches or updates them.

### Tests

The suite uses Node's built-in test runner, so there is nothing to `npm install`. `test/*.test.mjs` import the modules under `public/src/` directly, so the tests exercise the exact code the browser runs:
...
passes, and you've smoke-tested the affected surface in a browser (`npm start`, then exercise the direction, source language, or target script you touched). Don't claim "tested" beyond what the suite covers. The unit tests cover the engine and romanizers; UI wiring and lazy-loading of espeak, pinyin-pro, and kuroshiro are verified by hand, so say so and say how.

---

## Self-hosting and deploy
...
- **Any static host** (GitHub Pages, Netlify, S3, and the like). Serve the `public/` folder.
...
- **Plain ES modules, with no framework, transpiler, or bundler.** Source files in `public/src/` are served to the browser verbatim, and the tests import those same files. Keep them runnable in both the browser and `node --test`: no bundler-only syntax, and no `node_modules` imports inside `public/`.
...
- **Client-side only.** No backend and no external API calls; everything runs in the browser. The dictionaries and engines are vendored and lazy-loaded, so don't reach for a network service.
...
- Start each bullet with the area, followed by a colon: `Forward: `, `Romanize: `, `Engine: `, `UI: `.
