...
acknowledge reports within a few days. There is no bug-bounty program, since this is
...
- **XSS or HTML injection** via how input text, the romanized result, or the
  transliterated output is rendered into the DOM. Any user-supplied string, in
  any script, that escapes text context and executes or injects markup is a
  bug we want.
- **Unexpected network requests or data exfiltration.** The app must make no
  ...
  a third party, that's a vulnerability, so please report it.
...
These are documented properties of a phonetic-transliteration toy, not bugs, so
