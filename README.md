...
- **Latin-script language → world scripts.** Type one of 14 languages
  ...
  `300` becomes "three hundred", and so do decimals, years, ordinals, currency,
...
This is *phonetic* transliteration: it reproduces how text sounds, not what it
means. So "coffee" becomes コーヒー-style カフィー, not the Japanese word for coffee.

## Screenshots

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/screenshot-dark.png" />
  <img alt="Transliterate Me, the forward (Latin script to world scripts) view" src="docs/screenshot-light.png" />
</picture>

<sub>The forward view (Latin script → world scripts). It switches with your GitHub light/dark theme.</sub>


> **Setup note:** the two large binaries, espeak-ng's WASM and the kuromoji
> dictionary (~36 MB together), are committed in this repo (`public/vendor/`),
> so it works out of the box. Clone or download it and the espeak languages and
> Japanese kanji→romaji work with no extra steps. `./vendor.sh` (needs Node/npm)
> is only there to re-fetch or update those binaries.
...
1. **Text → pronunciation.** For English, each word is looked up in the
   ...
   rule-based fallback (`src/g2p.js`), which is instant and offline. For every other
   ...
   percentages, and fractions. English uses a built-in number speller; the
   other 13 languages use espeak's native per-voice expansion.
...
Everything runs client-side in the browser. There is no server-side processing and
...
| --- | --- | --- |
| Japanese (Katakana) | good | Loanword-style adaptation with long vowels and palatalised mora. |
...
| Chinese | fair | Logographic, so this is a nearest-Mandarin-syllable guess. It now emits only valid syllables mapped to conventional transcription characters (Smith → 斯米斯), with no pinyin leakage. |
...
No, and not for speed either. The whole pipeline is dictionary and table lookups
(sub-millisecond per word); client-side is actually *faster* for interactive use
because there is no network round-trip per keystroke, and it is free to host as
static assets.
...
- **Cloudflare Workers** (this repo's connected integration). `wrangler.toml`
  ...
  `npx wrangler deploy`.
- **Any static host** (GitHub Pages, Netlify, S3, and the like). Serve the
  `public/` folder.
...
**[CONTRIBUTING.md](.github/CONTRIBUTING.md)** for setup, the test and
golden-output conventions, the registry-based extension points (how to add a
language, script, or romanizer), and the house rules. The main one: keep it
client-side, dependency-free, plain ES modules, and keep the vendored license
notices intact.
